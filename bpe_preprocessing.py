...
def count_pairs(ids):
    ...
    for pair in zip(
        ids, ids[1:]
    ):  # zip function returns the pairs and together with (ids,ids[1:]) basically returns consecutive elements
        count[pair] = (
            count.get(pair, 0) + 1
        )  # pair is the key, get(key,0) returns the value of the key, if the key doesnt exist, gives 0 as value
    return count
